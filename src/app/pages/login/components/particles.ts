...
export const particles = (function () {
  ...
  function launchParticlesJS(tagId, params) {

    let canvasEl = <HTMLCanvasElement> document.querySelector('#' + tagId + ' > canvas');
    ...
    if (params) {
      if (params.particles) {
        let paramsForParticles = params.particles;
        ...
        if (!paramsForParticles.size_random) {
        ...
        if (paramsForParticles.line_linked) {
          let paramsForLineLinked = paramsForParticles.line_linked;
          if (!paramsForLineLinked.enable_auto) {
          ...
          if (paramsForLineLinked.condensed_mode) {
            let paramsForCondensedMode = paramsForLineLinked.condensed_mode;
            if (!paramsForCondensedMode.enable) {
        ...
        if (paramsForParticles.anim) {
          let paramsForAnim = paramsForParticles.anim;
          if (!paramsForAnim.enable) {
      ...
      if (params.interactivity) {
        let paramsForInteractivity = params.interactivity;
        if (!paramsForInteractivity.enable) {
        ...
        if (paramsForInteractivity.events) {
          let paramsForEvents = paramsForInteractivity.events;
          if (paramsForEvents.onclick) {
            let paramsForOnclick = paramsForEvents.onclick;
            if (!paramsForOnclick.enable) {
    ...
    pJS.fn.particle = function (color, opacity, position) {
      ...
      this.draw = function () {
        ...
        pJS.canvas.ctx.fillStyle = `rgba(${r}, ${g}, ${b}, ${this.opacity})`;
    ...
    pJS.fn.particlesCreate = function () {
      for (let i = 0; i < pJS.particles.nb; i++) {
    ...
    pJS.fn.particlesDraw = function () {
      ...
      /* draw each particle */
      for (let p of pJS.particles.array) {
    ...
    pJS.fn.vendors.distanceParticles = function (p1, p2) {
      ...
      if (dist <= pJS.particles.line_linked.distance) {
        ...
        /* draw the line */
        let colorLine = pJS.particles.line_linked.color_rgb_line;
        pJS.canvas.ctx.beginPath();
        let { r, g, b } = colorLine;
        let opacity = pJS.particles.line_linked.opacity - dist / pJS.particles.line_linked.distance;
        pJS.canvas.ctx.strokeStyle = `rgba(${r},${g},${b},${opacity})`;
        ...
        if (pJS.particles.line_linked.condensed_mode.enable) {
          let _dx = p1.x - p2.x;
          let _dy = p1.y - p2.y;
          let ax = _dx / (pJS.particles.line_linked.condensed_mode.rotateX * 1000);
          let ay = _dy / (pJS.particles.line_linked.condensed_mode.rotateY * 1000);
          p2.vx += ax;
          p2.vy += ay;
        }
    ...
    pJS.fn.vendors.interactivity.listeners = function () {
      ...
      if (pJS.interactivity.detect_on === 'window') {
        ...
        detectEl = pJS.canvas.el;
      }

      /* el on mousemove */
      detectEl.onmousemove = function (e) {
      ...
      /* el on onmouseleave */
      detectEl.onmouseleave = function (e) {

        if (pJS) {
          pJS.interactivity.mouse.posX = 0;
          pJS.interactivity.mouse.posY = 0;
      ...
      if (pJS.interactivity.events.onclick.enable) {
        switch (pJS.interactivity.events.onclick.mode) {
          case 'push':
            detectEl.onclick = function (e) {
              if (pJS) {
                for (let i = 0; i < pJS.interactivity.events.onclick.nb; i++) {
                  pJS.particles.array.push(
                    new pJS.fn.particle(
                      pJS.particles.color_rgb,
                      pJS.particles.opacity,
                      {
                        x: pJS.interactivity.mouse.posX,
                        y: pJS.interactivity.mouse.posY
                      }
                    )
                  );
                }
    ...
    pJS.fn.vendors.interactivity.grabParticles = function (p1, p2) {
      ...
      let dxMouse = p1.x - pJS.interactivity.mouse.posX;
      let dyMouse = p1.y - pJS.interactivity.mouse.posY;
      let distMouse = Math.sqrt(dxMouse * dxMouse + dyMouse * dyMouse);

      /* Check distance between 2 particles
        + Check distance between 1 particle and mouse position */
      if (dist <= pJS.particles.line_linked.distance
          && distMouse <= pJS.interactivity.mouse.distance
          && pJS.interactivity.status === 'mousemove') {
        /* Draw the line */
        let colorLine = pJS.particles.line_linked.color_rgb_line;
        let opacity = pJS.interactivity.line_linked.opacity
          - distMouse / pJS.interactivity.mouse.distance;
        pJS.canvas.ctx.beginPath();
        pJS.canvas.ctx.strokeStyle = 'rgba(' + colorLine.r + ',' + colorLine.g
          + ',' + colorLine.b + ','
          + opacity + ')';
        pJS.canvas.ctx.moveTo(p1.x, p1.y);
  ...
  let particlesJS = function (tagId, params) {
    ...
    /* append canvas */
    let canvas = document.getElementById(tagId).appendChild(canvasEl);

    /* launch particle.js */
    if (canvas != null) {
      return launchParticlesJS(tagId, params);
    }
